...
class _DashboardLoadedState extends ConsumerState<_DashboardLoaded> {
  ...
  void _showStateTooltip(StateAnalytics data) {
    ...
    // Same as web: map clicks still select a state for every role, including
    // when Territory permission is off. Pipeline / pies stay on overall KPIs.
    ref.read(territoryFiltersProvider.notifier).toggleState(data.name);
  }

  @override
  Widget build(BuildContext context) {
    // Keep web order: Territory (if granted) → India map → Pipeline → pies.
    // Territory permission only hides the State/District/Role/User card.
    // Map, pipeline, and pies always render with overall/total data.
          ...
          children: [
            ...
            Text(
              canFilterByState
                  ...
                  : 'Tap a state on the map or list to see its summary.',
              style: Theme.of(context).textTheme.labelSmall?.copyWith(
                    color: Theme.of(context).colorScheme.onSurfaceVariant,
                  ),
            ),
